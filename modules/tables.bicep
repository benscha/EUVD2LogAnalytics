    ...
    dataFlows: [
      {
        ...
        transformKql: 'source | extend CveId = coalesce(extract(@"CVE-\\d{4}-\\d+", 0, tostring(Aliases)), ""), GHSAId = coalesce(extract(@"GHSA-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}-[A-Za-z0-9]{4}", 0, tostring(Aliases)), "")'
      }
    ]
  }
}
